...
public class FastCollinearPoints {
    ...
    private void addSegment(Stack<Point> collinearPoints) {
        Point[] arr = new Point[collinearPoints.size()];
        int i = 0;
    ...
    private void validateInput(Point[] points) {
        ...
        for (Point point : points) {
            if (point == null) {
                throw new IllegalArgumentException("The input cannot contain null points");
            }
        }
        ...
        for (int i = 0; i < points.length; i++) {
            for (int j = i + 1; j < points.length; j++) {
                if (points[i].compareTo(points[j]) == 0) {
                    throw new IllegalArgumentException("The input cannot contain repeated points");
                }
            }
        }
    }
    ...
    private class Segment {
        ...
        public boolean equals(Segment other) {
            return ((minPoint.compareTo(other.minPoint) == 0) && (maxPoint.compareTo(other.maxPoint)
                    == 0));
        }
    }
}
